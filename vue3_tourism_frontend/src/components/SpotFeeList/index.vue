...
import type * as API from "@/api/interface";
import {
  buySpotFeeUsingPost,
...
// 门票列表数据
const spotFeeList = ref<API.SpotFeeVO[]>([]);

// 购票对话框状态
const buyDialogVisible = ref(false);

// 当前选中的门票
const selectedTicket = ref<Partial<API.SpotFeeVO>>({
  id: 0,
  spotFeePrice: 0,
  spotFeeDescription: "",
  createTime: "",
...
const getSpotFeeList = async () => {
  try {
    ...
    if (res.data.code === 200) {
      ...
      spotFeeList.value = res.data.data?.records ?? [];
    } else {
      ElMessage.error(res.data.message || "获取门票列表失败");
    }
  } catch (error) {
...
// 打开购票对话框
const showBuyDialog = (item: API.SpotFeeVO) => {
...
const calculateTotalPrice = () => {
  totalPrice.value =
    (selectedTicket.value.spotFeePrice ?? 0) * buyForm.value.quantity;
};
...
const submitBuyOrder = async () => {
  try {
    ...
    if (res.data.code === 200) {
      ...
      ElMessage.error(res.data.message || "购票失败");
    }
  } catch (error) {
